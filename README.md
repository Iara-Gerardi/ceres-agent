...
Set `AI_GATEWAY_API_KEY`, `DATABASE_URL`, and `LINKUP_API_KEY` in `.env` before starting the agent.
...
4. Add `AI_GATEWAY_API_KEY`, `LINKUP_API_KEY`, `ROUTE_AUTH_BASIC_USER`, and a strong `ROUTE_AUTH_BASIC_PASSWORD` to the agent service variables.
...
The `query_database` tool accepts parameterized queries and enforces a read-only PostgreSQL connection. It only permits a single `SELECT`, `WITH`, or `EXPLAIN` statement. The `linkup_search` tool provides cited, real-time web results through the Linkup Search API.npx skills add LinkupPlatform/skills